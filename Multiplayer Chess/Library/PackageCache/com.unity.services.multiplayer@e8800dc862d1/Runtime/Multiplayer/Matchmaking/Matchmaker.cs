    ...
    class Matchmaker : ISessionMatchmaking
    {
        ...
        async Task PollTicketStatusAsync()
        {
            try
            {
                    ...
                    ticketResponse.Value is NoneAssignment noneAssignment)
                {
                    ...
                    switch (noneAssignment.Status)
                    {
                        ...
                        case NoneAssignment.StatusOptions.Failed:
                            var failedMessage = $"Ticket {noneAssignment.Status}: {(string.IsNullOrEmpty(noneAssignment.Message) ? k_AssignmentFailedMessage : noneAssignment.Message)}";
                            SetMatchFailure(failedMessage, SessionError.MatchmakerAssignmentFailed);
                            return;
                        case NoneAssignment.StatusOptions.Timeout:
                            var timeoutMessage = $"Ticket {noneAssignment.Status}: {(string.IsNullOrEmpty(noneAssignment.Message) ? k_AssignmentTimeoutMessage : noneAssignment.Message)}";
                    ...
                    ticketResponse.Value is CustomAssignment customAssignment)
                {
                    ...
                    switch (customAssignment.Status)
                    {
                        ...
                        case CustomAssignment.StatusOptions.Failed:
                            var failedMessage = $"Ticket {customAssignment.Status}: {(string.IsNullOrEmpty(customAssignment.Message) ? k_AssignmentFailedMessage : customAssignment.Message)}";
                            SetMatchFailure(failedMessage, SessionError.MatchmakerAssignmentFailed);
                            return;
                        case CustomAssignment.StatusOptions.Timeout:
                            var timeoutMessage = $"Ticket {customAssignment.Status}: {(string.IsNullOrEmpty(customAssignment.Message) ? k_AssignmentTimeoutMessage : customAssignment.Message)}";
                            SetMatchFailure(timeoutMessage, SessionError.MatchmakerAssignmentTimeout);
                            return;
                        default:
                            return;
                    ...
                    ticketResponse.Value is IpPortAssignment ipPortAssignment)
                {
                    ...
                    switch (ipPortAssignment.Status)
                    {
                        ...
                        case IpPortAssignment.StatusOptions.Failed:
                            var failedMessage = $"Ticket {ipPortAssignment.Status}: {(string.IsNullOrEmpty(ipPortAssignment.Message) ? k_AssignmentFailedMessage : ipPortAssignment.Message)}";
                            SetMatchFailure(failedMessage, SessionError.MatchmakerAssignmentFailed);
                            return;
                        case IpPortAssignment.StatusOptions.Timeout:
                            var timeoutMessage = $"Ticket {ipPortAssignment.Status}: {(string.IsNullOrEmpty(ipPortAssignment.Message) ? k_AssignmentTimeoutMessage : ipPortAssignment.Message)}";
                            SetMatchFailure(timeoutMessage, SessionError.MatchmakerAssignmentTimeout);
                            return;
                        default:
                            return;
                ...
                var message = $"{nameof(m_MatchmakerService.GetTicketAsync)} returned an invalid assignment type. This operation is not supported.";
